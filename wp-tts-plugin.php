...
define('WP_TTS_PLUGIN_VERSION', '1.9.1');
...
function wp_tts_plugin_action_links($links) {
    $settings_link = '<a href="' . esc_url( admin_url('options-general.php?page=wp-tts-settings') ) . '">' . esc_html__('Settings', 'tts-sesolibre') . '</a>';
...
function wp_tts_plugin_meta_links($links, $file) {
    if ($file === WP_TTS_PLUGIN_BASENAME) {
        $links[] = '<a href="https://github.com/ellaguno/tts_de_wordpress/wiki" target="_blank">' . esc_html__('Documentation', 'tts-sesolibre') . '</a>';
        $links[] = '<a href="https://github.com/ellaguno/tts_de_wordpress/issues" target="_blank">' . esc_html__('Support', 'tts-sesolibre') . '</a>';
    }
    return $links;
}
