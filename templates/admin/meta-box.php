        ...
        <div class="wp-tts-field-header">
            <button type="button" id="audio-assets-header" aria-expanded="false" aria-controls="audio-assets-content"
                ...
                <span class="wp-tts-field-label" style="font-weight:600;">
                    ...
                    <small style="color: #666; font-weight: normal;"><?php esc_html_e('(clic para expandir)', 'tts-sesolibre'); ?></small>
                </span>
            </button>
        </div>
    ...
    <div class="wp-tts-field wp-tts-conditional" data-depends="tts_enabled">
        ...
        <div class="wp-tts-field-content">
                    ...
                    <div id="tts_generation_progress" style="display: none; margin-top: 15px;">
                        ...
                        <p class="wp-tts-progress-text"><?php esc_html_e('Generando audio… puede tardar hasta un minuto según la longitud del artículo.', 'tts-sesolibre'); ?></p>
                    </div>
...
jQuery(document).ready(function($) {
    ...
    function initializeVoices() {
        ...
        if (provider) {
            ...
            $voiceSelect.html('<option value=""><?php echo esc_js(__("Loading voices...", "tts-sesolibre")); ?></option>');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $voiceSelect.html('<option value=""><?php echo esc_js(__("Error loading voices", "tts-sesolibre")); ?></option>');
                    }
                },
                error: function() {
                    $voiceSelect.html('<option value=""><?php echo esc_js(__("Error loading voices", "tts-sesolibre")); ?></option>');
    ...
    $('#tts_voice_provider').on('change', function() {
        ...
        if (!provider) {
            $voiceSelect.html('<option value=""><?php echo esc_js(__("Use default voice", "tts-sesolibre")); ?></option>');
            return;
        }
    ...
    $('#tts_generate_now, #tts_regenerate').on('click', function() {
        ...
        if (!postId) {
            alert('<?php echo esc_js(__("Please save the post first", "tts-sesolibre")); ?>');
            return;
        }
    ...
    $('#tts_delete_audio').on('click', function() {
        if (!confirm('<?php echo esc_js(__("Are you sure you want to delete the generated audio? This action cannot be undone.", "tts-sesolibre")); ?>')) {
            return;
        }
        ...
        $button.prop('disabled', true).html('<span class="dashicons dashicons-update wp-tts-spin"></span> <?php echo esc_js(__("Deleting...", "tts-sesolibre")); ?>');
    ...
    $(document).on('click', '.wp-tts-field .tts-select-media', function(e) {
        ...
        // Create media frame
        mediaFrame = wp.media({
            title: '<?php echo esc_js(__("Select Audio File", "tts-sesolibre")); ?>',
            button: {
                text: '<?php echo esc_js(__("Use this audio", "tts-sesolibre")); ?>'
            },
            library: {
                type: 'audio'
            },
        ...
        mediaFrame.on('select', function() {
            ...
            var audioHtml = '<audio controls style="width: 100%; margin-bottom: 10px;">' +
                ...
                '<?php echo esc_js(__("Your browser does not support the audio element.", "tts-sesolibre")); ?>' +
                '</audio>';
    ...
    $('#tts_edit_text').on('click', function() {
        ...
        // Show loading
        $(this).prop('disabled', true).html('<span class="dashicons dashicons-update wp-tts-spin"></span> <?php echo esc_js(__("Loading...", "tts-sesolibre")); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php echo esc_js(__("Error extracting content", "tts-sesolibre")); ?>');
                }
            },
            error: function() {
                alert('<?php echo esc_js(__("Error connecting to server", "tts-sesolibre")); ?>');
            },
            complete: function() {
                $('#tts_edit_text').prop('disabled', false).html('<span class="dashicons dashicons-edit"></span> <?php echo esc_js(__("Edit Text Before Generate", "tts-sesolibre")); ?>');
            }
        });
    });
            ...
            <div id="tts-editor-modal" class="tts-modal-backdrop">
                <div class="tts-modal-container">
                    <div class="tts-modal-header">
                        <h2><?php echo esc_js(__("Edit Text for TTS Generation", "tts-sesolibre")); ?></h2>
                        <span class="tts-modal-close">&times;</span>
                    </div>
                    <div class="tts-modal-body">
                        <div class="tts-editor-info">
                            <strong><?php echo esc_js(__("Post:", "tts-sesolibre")); ?></strong> ${data.post_title}
                        ...
                        <div class="tts-editor-toolbar">
                            <button type="button" id="tts-clean-text" class="button button-small">
                                <span class="dashicons dashicons-admin-tools"></span> <?php echo esc_js(__("Clean Text", "tts-sesolibre")); ?>
                            </button>
                            <button type="button" id="tts-reset-text" class="button button-small">
                                <span class="dashicons dashicons-undo"></span> <?php echo esc_js(__("Reset to Original", "tts-sesolibre")); ?>
                            </button>
                        </div>
                        <textarea id="tts-editor-textarea" rows="15" class="large-text">${data.text}</textarea>
                        <div class="tts-editor-stats">
                            <span id="tts-char-count">${data.character_count}</span> <?php echo esc_js(__("characters", "tts-sesolibre")); ?>
                            <span style="margin: 0 10px;">|</span>
                            <span id="tts-word-count">${data.word_count}</span> <?php echo esc_js(__("words", "tts-sesolibre")); ?>
                            <span style="margin: 0 10px;">|</span>
                            <span id="tts-cost-estimate">$${ttsEstimateEditorCost(data.character_count)}</span> <?php echo esc_js(__("costo aproximado", "tts-sesolibre")); ?>
                        </div>
                    </div>
                    <div class="tts-modal-footer">
                        <button type="button" id="tts-save-and-generate" class="button button-primary">
                            <span class="dashicons dashicons-controls-play"></span> <?php echo esc_js(__("Save & Generate Audio", "tts-sesolibre")); ?>
                        </button>
                        <button type="button" id="tts-save-only" class="button button-secondary">
                            <span class="dashicons dashicons-saved"></span> <?php echo esc_js(__("Save Only", "tts-sesolibre")); ?>
                        </button>
                        <button type="button" class="button tts-modal-close"><?php echo esc_js(__("Cancel", "tts-sesolibre")); ?></button>
                    </div>
                </div>
            </div>
        `;
    ...
    $(document).on('click', '#tts-reset-text', function() {
        if (originalPostText && confirm('<?php echo esc_js(__("Are you sure you want to reset to the original text? All changes will be lost.", "tts-sesolibre")); ?>')) {
    ...
    $(document).on('click', '#tts-save-only', function() {
        ...
        if (!text) {
            alert('<?php echo esc_js(__("Text cannot be empty", "tts-sesolibre")); ?>');
            return;
        }
        
        const $button = $(this);
        const originalText = $button.html();
        $button.prop('disabled', true).html('<span class="dashicons dashicons-update wp-tts-spin"></span> <?php echo esc_js(__("Saving...", "tts-sesolibre")); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    let message = '<?php echo esc_js(__("Text saved successfully!", "tts-sesolibre")); ?>';
                    message += '\n<?php echo esc_js(__("Characters:", "tts-sesolibre")); ?> ' + charCount;
                    message += '\n<?php echo esc_js(__("Words:", "tts-sesolibre")); ?> ' + wordCount;
                    message += '\n\n<?php echo esc_js(__("You can now use the Generate Audio button to create TTS from your edited text.", "tts-sesolibre")); ?>';
                    alert(message);
                    $('#tts-editor-modal').remove();
                } else {
                    const errorMsg = response.data?.message || response.message || '<?php echo esc_js(__("Error saving text", "tts-sesolibre")); ?>';
                    alert('<?php echo esc_js(__("Save failed:", "tts-sesolibre")); ?>\n\n' + errorMsg);
                }
            },
            error: function(xhr, status, error) {
                alert('<?php echo esc_js(__("Error connecting to server:", "tts-sesolibre")); ?> ' + error);
            },
            complete: function() {
    ...
    $(document).on('click', '#tts-save-and-generate', function() {
        ...
        const $button = $(this);
        const originalText = $button.html();
        $button.prop('disabled', true).html('<span class="dashicons dashicons-update wp-tts-spin"></span> <?php echo esc_js(__("Saving & Generating...", "tts-sesolibre")); ?>');
        ...
        $.ajax({
                    ...
                    $.ajax({
                        ...
                        success: function(genResponse) {
                            if (genResponse.success) {
                                ...
                                let message = '<?php echo esc_js(__("Audio generated successfully!", "tts-sesolibre")); ?>';
                                if (audioUrl) {
                                    message += '\n<?php echo esc_js(__("Audio URL:", "tts-sesolibre")); ?> ' + audioUrl;
                                }
                                alert(message);
                                ...
                                const errorMsg = genResponse.data?.message || genResponse.message || '<?php echo esc_js(__("Error generating audio", "tts-sesolibre")); ?>';
                                alert('<?php echo esc_js(__("Generation failed:", "tts-sesolibre")); ?> ' + errorMsg);
                            }
                        },
                        error: function(xhr, status, error) {
                            alert('<?php echo esc_js(__("Error generating audio:", "tts-sesolibre")); ?> ' + error);
                        },
                        complete: function() {
                            $button.prop('disabled', false).html(originalText);
                        }
            ...
            error: function(xhr, status, error) {
                alert('<?php echo esc_js(__("Error saving text", "tts-sesolibre")); ?>');
                $button.prop('disabled', false).html(originalText);
            }
        });
    });
