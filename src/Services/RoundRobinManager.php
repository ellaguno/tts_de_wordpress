...
class RoundRobinManager {
	...
	public function __construct( ConfigurationManager $config ) {
		$this->config = $config;
		$this->current_index = max( 0, (int) get_option( 'wp_tts_round_robin_index', 0 ) );
	}
	...
	public function getNextProvider(): ?string {
		...
		// Re-normalize the persisted index: the provider set may have shrunk
		// since it was saved, leaving it out of bounds.
		$this->current_index = $this->current_index % count( $providers );
	...
	public function getCurrentProvider(): ?string {
		...
		return $providers[ $this->current_index % count( $providers ) ];
	}
